...
def backup_tables(connection):
    ...
    # 备份students表
    cursor.execute("CREATE TABLE students_backup_{} AS SELECT * FROM students".format(timestamp))
    
    # 备份registration_applications表  
    cursor.execute("CREATE TABLE registration_applications_backup_{} AS SELECT * FROM registration_applications".format(timestamp))
    
    print(f"✅ 备份完成，备份表: students_backup_{timestamp}, registration_applications_backup_{timestamp}")
    return timestamp
...
def migrate_remove_class_fields():
    ...
    try:
        ...
        # 备份数据
        backup_timestamp = backup_tables(connection)
        print("🗂️  开始删除字段...")
        ...
        # 检查registration_applications表的外键约束
        cursor.execute("""
            SELECT CONSTRAINT_NAME 
            FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
            WHERE TABLE_SCHEMA = 'education_system' 
        ...
        # 检查并删除registration_applications表中的class_id字段
